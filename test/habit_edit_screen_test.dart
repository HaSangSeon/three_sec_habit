...
void main() {
  testWidgets('새 습관 생성 폼 렌더링 및 주요 입력 컴포넌트 검증', (WidgetTester tester) async {
    ...
    expect(find.text('새 습관'), findsOneWidget);
    expect(find.text('습관 이름'), findsOneWidget);
    expect(find.text('목표 횟수 (선택)'), findsOneWidget);
    expect(find.text('아이콘 및 색상'), findsOneWidget);
    expect(find.text('반복 주기'), findsOneWidget);
    expect(find.text('알림 설정'), findsOneWidget);
    expect(find.text('오늘의 습관'), findsOneWidget);
    expect(find.text('통계/기록'), findsOneWidget);
